...
from blip_vqa_pipeline.pipeline import (
    MANIFEST_NAME,
    MODEL_ID,
    MODEL_REVISION,
    BlipVQAPipeline,
...
_CONFIG = json.dumps({"model_type": "blip"}).encode()
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        BlipVQAPipeline.from_pretrained(device="cpu", weights_dir=tmp_path, allow_download=False)
