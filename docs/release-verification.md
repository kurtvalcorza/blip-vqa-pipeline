...
`tutorials/blip_vqa_colab.ipynb` (`TASK-INFERENCE`, **standalone** carrier) is a
...
- the standalone carrier (ST1–ST6, PAR1–PAR3): no clone, repository install or repository import on
  ...
  `src/blip_vqa_pipeline/pipeline.py` after the generator's documented rewrites; the
...
- the profile-specific public-API calls (`stage_missing_files`, `verify_snapshot`,
  `BlipVQAPipeline.from_pretrained(weights_dir=...)`, `validate_inputs`, `answer`,
  `evaluation_report`), the ceiling print (`MIN_IMAGE_SIDE`, `MAX_IMAGE_SIDE`, `IMAGE_SIZE`,
  ...
  no VQA v2 benchmark, exact match / ANLS as sanity check, the model answers any question about any image,
  ...
  `revision='main'`, direct `from transformers import` / `BlipForQuestionAnswering` / `BlipProcessor` /
  `model.generate(` / `from huggingface_hub import` use **outside the carried module cell**,
...
3. run the notebook top-to-bottom without editing implementation cells (form parameters at their
   defaults for the sample path: `USE_BYOD = False`, `max_new_tokens = 10`);
...
5. verify every default-path stage completes:
   ...
   - the carried module cell executes (defines `BlipVQAPipeline`, `validate_inputs`,
     `evaluation_report`, `exact_match`, `anls`, `vqa_accuracy`, `verify_snapshot`,
     `stage_missing_files`) with no import of the repository package;
   - synthetic 640×480 cartoon scene drawn in code with its RGB SHA-256 printed and the ceilings
     (`MIN_IMAGE_SIDE` 16, `MAX_IMAGE_SIDE` 4096, `IMAGE_SIZE` 384, `MAX_QUESTION_CHARS` 256,
     `MAX_NEW_TOKENS` 32, `DEFAULT_MAX_NEW_TOKENS` 10, `DECODING` greedy) surfaced;
   - pinned `Salesforce/blip-vqa-base` acquisition at the immutable revision through the carried
     module: the inline `MANIFEST` is asserted against the module identity and written to
     `weights/blip-vqa-base/`, `stage_missing_files(WEIGHTS_DIR, allow_download=True)` reports
     all 8 manifest entries on a clean runtime, `verify_snapshot` returns its summary dict, and
     `from_pretrained(weights_dir=WEIGHTS_DIR)` loads from the verified directory with no further
     Hub access (any download in the logs after staging is a finding);
   - `validate_inputs` writes `outputs/blip_vqa_input_manifest.json` (verdict `accepted`, seven
   ...
   - `answer` returning one answer per question with `truncated` false on the default budget; record the
     answers (the card-pass CPU smoke answered `red`, `1`, `sun`, `tree`, `blue`, `2`, `red` — the last
     two are the recorded misses against the drawn `1` tree and brown door; a materially different result
     is a finding to record, not a failure by itself, because no metric is asserted — greedy decoding on
     a different device can diverge);
   - `evaluation_report` writes `outputs/blip_vqa_evaluation_report.json` with verdict
     `sample-sanity`, an `exact_match` entry, an `anls` entry and seven per-question entries on the
     synthetic sample (`not-measurable` on BYOD), stated as such;
   - `outputs/blip_vqa_result.json`, `outputs/blip_vqa_answers.csv` and
     `outputs/blip_vqa_annotated.png` written with `NOTEBOOK_SOURCE`, model revision, model
...
`git rev-parse <commit>:tutorials/blip_vqa_colab.ipynb`). Wall times, when recorded,
...
|---|---|---|---|---|---|
| 2026-09-14 | notebook blob `81541d70ad87` (commit `43a1149`, generated at `430a3f3`; `NOTEBOOK_SOURCE.repository_revision` = `430a3f3…`) | Local Windows-venv harness (`run_nb_local.py`: nbclient 0.11.0, fresh `python3` kernel, `CUDA_VISIBLE_DEVICES=-1`, `DIMER_NOTEBOOK_CI_PREINSTALLED=1`), Python 3.12.10, torch 2.14.0+cu130, transformers 4.57.6 | Default synthetic path, all 8 code cells: pinned install skipped (pre-installed), `stage_missing_files` fetched all 8 manifest entries (1.54 GB) from the Hub cache at the pinned revision into the scratch `weights/`, `verify_snapshot` PASS (8 files), no further download in the log, seven `answer` calls → `red`, `1`, `sun`, `tree`, `blue`, `2`, `red` (2 tokens each, none truncated, 0.18–0.48 s), `evaluation_report` `sample-sanity` (`exact_match` 0.714 = 5/7, `anls` 0.714; the two recorded misses reproduced), scene digest `f217010b…`, 5 outputs written | 148.1 s | PASS — pre-flight only; not promotion evidence |

### Manual clean-runtime evidence

| Date (UTC) | Commit / notebook blob | Executor | Path exercised | Wall | Outcome |
|---|---|---|---|---|---|
| 2026-09-14 | `89fd58c` / `821ff44cd8f5` | Kaggle CPU (`kurtvalcorza/dimer-nb2-blip-vqa` v1) | Default sample path | 295.2 s | **PASSED** — 8/8 ok code cells executed cleanly, 18 files, 1540 MB staged |
...
path has not been executed; the tutorial sample is a flat cartoon, not a photograph, and the model's two
recorded misses on it (`2` trees, `red` door) are kept in the scored set on purpose; on blank white
images the model answered `dog` and `white` and on uniform noise `tv`, so an unanswerable question yields
a fluent invented answer with no signal; every image is resized to 384×384 without preserving aspect
ratio; and each question costs an encoder pass (~0.2 s on the reference CPU), so a long question list
scales linearly.
